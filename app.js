...
async function getNews() {
    ...
    const apollo = await fetch(APOLLO_RSS_URL);
    const tvnet = await fetch(TVNET_RSS_URL);
    const delfi = await fetch(DELFI_RSS_URL);
    const lsm = await fetch(LSM_RSS_URL);
    ...
    const apollo_result = await parser.parseString(apollo_text);
    const tvnet_result = await parser.parseString(tvnet_text);
    const delfi_result = await parser.parseString(delfi_text);
    const lsm_result = await parser.parseString(lsm_text);
